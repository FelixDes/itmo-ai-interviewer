...
presigned-загрузка в MinIO, выравнивание техжаргона. И вся ИИ-часть: вопросы,
уточнения, оценка ответов и текст карточки пишет модель через API DeepSeek.

Вердикты по требованиям, итоговый балл и рекомендацию по-прежнему считают
правила — числа должны быть воспроизводимыми, модель их не трогает.

### Что ещё заглушено

| Поведение | Как сейчас | Как будет |
|---|---|---|
| Распознавание речи | Правдоподобный русский текст по теме вопроса, задержка 2,5 с | NeMo на `asr:8100` |
...
| Пауза между вопросами | 5–8 с | **Кандидат**, на каждый ответ |
