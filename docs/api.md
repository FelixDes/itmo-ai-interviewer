...
Персистентность, разграничение доступа, стейт-машина, версионирование наборов,
presigned-загрузка в MinIO, выравнивание техжаргона в транскрипте, правила
вердиктов по требованиям и расчёт итогового балла. Данные переживают рестарт.

### Что ещё заглушено

| Поведение | Как сейчас | Как будет |
|---|---|---|
| Распознавание речи | Правдоподобный русский текст с таймкодами, задержка 2,5 с | NeMo на `asr:8100` |
| Генерация вопросов | `LLM` и `REFERENCE` дают одни и те же 6 эталонных вопросов | LLM по вакансии |
| Персональные вопросы | 2 шаблонных, с фрагментом резюме внутри | LLM по резюме |
| Оценка ответа | Правила по признакам: есть ли пример, личный вклад, числа | LLM по рубрике |
| Текст выводов в карточке | Формулировки собираются по шаблонам | LLM |
| Озвучка вопроса | WAV-тишина нужной длительности | Silero на `tts:8101` |

Задержку распознавания можно менять: `app.asr.stub-delay-ms` (по умолчанию 2500).
Ставьте 0, если ждать надоело.
...
type AntifraudEventType = "TAB_HIDDEN" | "COPY" | "PASTE";
```
...
Фронт слушает `visibilitychange`, `copy`, `paste` и отправляет событие
fire-and-forget. Ошибку отправки игнорирует — событие не должно ломать интервью.
